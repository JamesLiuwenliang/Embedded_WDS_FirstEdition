...
//Ô­À´ÊÇchar »¹ÓĞvolatile
static int g_ts_data_valid = 0;/*charÒ»´ÎĞÔ¾Í»á±»²ğ·ÖÍê,ÕâÀïÊ¹ÓÃvolatileµÄÄ¿µÄÊÇÖ±½Ó²Ù×÷ÄÚ´æ,±ÜÃâ±»ÓÅ»¯*/
...
void ts_read_raw(int *px,int *py,int *ppressure)/*¼ÇÂ¼´¥ÃşÆÁ×ø±ê*/
{
...
/*Õâ¸öº¯ÊıÃ¿¸ô10ms»áµ÷ÓÃÒ»´Î*/
void touchscreen_timer_irq()
{
	int y = ADCDAT1;

	/*Èç¹û´¥ÃşÆÁÈÔ±»°´ÏÂ,Æô¶¯ADC,½øÈë×Ô¶¯²âÁ¿Ä£Ê½*/
	...
	/* Ö»ÓĞÔÚµÈ´ıÖĞ¶ÏÄ£Ê½ÏÂ,²Å¿ÉÒÔÊ¹ÓÃADCDAT0bit'15À´²âÁ¿,
	 *Ô­ÎÄ: Up or Down state of stylus at waiting for interrupt mode. 
	 */
	if (ADCDAT0 & (1<<15))  /*ËÉ¿ª*/
	{
		ts_timer_disable();
		enter_wait_pen_down_mode();
		report_ts_xy(0,0,0);  /*ÉÏ±¨µ±Ç°Öµ*/
		return ;
	}
	else /*°´ÏÂ×´Ì¬*/
	{
...
void Isr_Adc(void)
{
	...
	if (!(ADCDAT0 & (1<<15))) /* Èç¹ûÈÔÈ»°´ÏÂ²Å´òÓ¡ */
	{
		/*µÚ1´ÎÆô¶¯ADCºóå:
		 * a. ÒªÁ¬ĞøÆô¶¯n´Î,ÔÙ»ñµÃn¸öÊı¾İ,ÇóÆ½¾ùÖµÔÙÉÏ±¨
		 * b. µÃµ½N´ÎÊı¾İºó,ÔÙÆô¶¯Timer
		 */
		adc_cnt ++;
		adc_x += ( ADCDAT0 & 0x3ff );
		adc_y += ( ADCDAT1 & 0x3ff );	
		...
		if(adc_cnt == 16)
		{
			...
			/*Æô¶¯¶¨Ê±Æ÷ÒÑÔÙ´Î¶ÁÈ¡Êı¾İ*/
			/*ÏÈÉèÖÃTS½øÈë'µÈ´ıÖĞ¶ÏÄ£Ê½'*/
			enter_wait_pen_up_mode();
			ts_timer_enable();
		}
		else 
...
#if 0
		/*ÕâÖÖ·½Ê½ÓĞÈ±Ïİ,×îºóËÉ¿ªµÄÒ»´Î¿ÉÄÜ»áÊÇÒ»´Î´íÎóµÄÊı¾İ¶øÇ°ÃæµÄÊı¾İ±»ÈÓµô,ËùÒÔ×îºóËã³öµÄ²ÎÊıÊÇÍêÈ«´íÎóµÄ*/
		x &= 0x3ff;
		y &= 0x3ff;
	...
	//enter_wait_pen_up_mode();//ÕâÀï¶à´ÎÒ»¾Ù,Æô¶¯ADCÊ±²»ÄÜ½øÈëÕâ¸öÄ£Ê½,»áÓ°ÏìÊı¾İ
}

void AdcTsIntHandle(int irq)
{
	if (SUBSRCPND & (1<<TC_INT_BIT))  /* Èç¹ûÊÇ´¥ÃşÆÁÖĞ¶Ï */
		Isr_Tc();

	if (SUBSRCPND & (1<<ADC_INT_BIT))  /* ADCÖĞ¶Ï */
		Isr_Adc();
